...
def pytest_addoption(parser):
    parser.addoption(
        "--repo", action="store_true", default=False, help=(
            "run integration tests that alter your real git repo")
    )
...
def pytest_collection_modifyitems(config, items):
    if config.getoption("--repo"):
        return
    skip_repo = pytest.mark.skip(reason=("specify --repo to run integration "
